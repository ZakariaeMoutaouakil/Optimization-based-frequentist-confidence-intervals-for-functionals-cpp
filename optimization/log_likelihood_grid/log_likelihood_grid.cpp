...
std::vector<std::vector<double> > log_likelihood_grid(const std::vector<std::vector<int> > &xs,
    ...
    const std::vector<double> maximize_product_xs = maximize_product_list(xs);

    // Calculate the maximized product for each fixed p_2 value using the `maximize_product_grid` function
    const std::vector<std::vector<double> > maximize_product_fixed_p2 = maximize_product_grid(xs, threshold, fixed_p2s);
